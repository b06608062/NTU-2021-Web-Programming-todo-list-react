...
class App extends Component {
  ...
  addItem = (e) => {
    ...
    if (e.code === "Enter" && todo !== "") {
      let idx = this.index;
      this.setState((s) => ({
        todo_items: [...s.todo_items, { item: todo, done: false, idx: idx }],
      }));
      this.index++;
      e.target.value = "";
    }
  ...
  deleteItem = (idx) => {
    this.setState((s) => ({
      todo_items: s.todo_items.filter((i) => i.idx !== idx),
    }));
  };
  ...
  clearCompleted = () => {
    this.setState((s) => ({
      todo_items: s.todo_items.filter((i) => i.done === false),
    }));
  };
